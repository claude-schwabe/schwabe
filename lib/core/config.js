...
// · whale (fable 5, the apex burn). Shorthands match the keys; `--model <id>`
// overrides any mode.
export const MODES = {
  ...
  whale:   { model: "claude-fable-5",             label: "WHALE",   icon: "🐋", blurb: "burns money for sport · fable 5 (Mythos-class)" },
};
...
export function parseConfig(argv = process.argv.slice(2)) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (key === "schwabe" || key === "student" || key === "rich" || key === "whale") { cfg.mode = key; continue; }
    if (key === "broke" || key === "peasant") { cfg.mode = "schwabe"; continue; }  // aliases (incl. legacy --peasant)
    if (key === "poor") { cfg.mode = "student"; continue; }
    if (key === "baller") { cfg.mode = "rich"; continue; }
    if (key === "fable" || key === "oligarch") { cfg.mode = "whale"; continue; }
